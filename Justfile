...
export repo_organization := env("GITHUB_REPOSITORY_OWNER", "barqos")
...
build $image=default_image $tag=default_tag $flavor=default_flavor $rechunk="false" $ghcr="false" $kernel_pin="" $retry_pull="false":
    ...
    LABELS+=("--label" "io.artifacthub.package.keywords=bootc,fedora,barq,kde,linux")
    LABELS+=("--label" "io.artifacthub.package.logo-url=https://avatars.githubusercontent.com/u/120078124?s=200&v=4")
    LABELS+=("--label" "io.artifacthub.package.readme-url=https://raw.githubusercontent.com/barqos/Image/refs/heads/main/README.md")
    LABELS+=("--label" "org.opencontainers.image.created=$(date -u +%Y\-%m\-%d\T%H\:%M\:%S\Z)")
    LABELS+=("--label" "org.opencontainers.image.description=A privacy-respecting, fast and stable KDE desktop")
    LABELS+=("--label" "org.opencontainers.image.documentation=https://github.com/barqos/Image")
    LABELS+=("--label" "org.opencontainers.image.source=https://raw.githubusercontent.com/barqos/Image/refs/heads/main/Containerfile.in")
    LABELS+=("--label" "org.opencontainers.image.title=${image_name}")
    LABELS+=("--label" "org.opencontainers.image.url=https://github.com/barqos/Image")
...
setup-cache $image=default_image $tag=default_tag $flavor=default_flavor $ghcr="false" $pull="false" $push="false" $registry="" $github_event="":
    ...
    BLESSED_IMAGE=barq-dx

    CACHE_NAME="barq/cache/dnf"
...
disk-image $image=default_image $tag=default_tag $flavor=default_flavor $ghcr="false" $backend="ostree":
    ...
    # this is enough for base barq to make it more likely to run in CI
    if [[ "${ghcr}" == "true" ]]; then
...
retag-nvidia-on-ghcr working_tag="" stream="" dry_run="1":
    ...
    for image in barq-nvidia-open barq-dx-nvidia-open; do
      $skopeo copy docker://ghcr.io/barqos/${image}:{{ working_tag }} docker://ghcr.io/barqos/${image}:{{ stream }}
    done
